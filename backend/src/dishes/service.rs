...
pub trait ReceiptsService {
    async fn receipts(user_id: UserId, get_owned: bool) -> Result<Vec<ReceiptSummary>, anyhow::Error>;
...
pub struct ReceiptServiceLive {}
impl ReceiptsService for ReceiptServiceLive {
    async fn receipts(user_id: UserId, get_owned: bool) -> Result<Vec<ReceiptSummary>, Error> {
        Ok(vec![])
    }
