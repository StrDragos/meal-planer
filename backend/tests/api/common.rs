...
pub fn spawn_app() -> String {
    ...
    let server = backend::run(listener, app_config).expect("Failed to start server");
    let _ = tokio::spawn(server);
    format!("http://127.0.0.1:{}", port)
}


//TODO: add a high oreder function to work with the container, make sure the container stays alive in the scope.
pub async fn firebase_auth_container() -> anyhow::Result<(ContainerAsync<GenericImage>, Option<u16>, Option<u16>)> {
    ...
    let ports = container.ports().await.expect("Cound not get available ports");
    ...
    Fut: Future<Output=()>,
{
    ...
    let ports = container.ports().await.expect("Couldn't get available ports");
    let auth_port = ports.map_to_host_port_ipv4(ContainerPort::Tcp(9099))
...
pub async fn sign_in(email: String, password: String, port: String) -> Result<String, io::Error> {
    ...
    let firebase_login = format!("http://localhost:{}/identitytoolkit.googleapis.com/v1/accounts:signUp?key=123", port);
    let mut attemts = 0;

    while attemts < 10 {
        let response = client.post(firebase_login.clone())
